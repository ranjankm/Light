...
public class HomeAdvView extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (category) {
            case "Best Deals":
                new AdvsList(HomeAdvView.this).execute("deals");
                break;
            case "Sales and Discounts":
                new AdvsList(HomeAdvView.this).execute("sales");
                break;
            case "New in the Market":
                new AdvsList(HomeAdvView.this).execute("new");
                break;
        }
    }

    protected void loadAdvs() {
        RecyclerView.LayoutManager shopLayoutManager = new LinearLayoutManager(HomeAdvView.this, LinearLayoutManager.VERTICAL, false);
    ...
    public class AdvsList extends AsyncTask<String, Void, JSONObject> {
        private static final String URL = "http://theoms.16mb.com/Advs_List.php";
        ProgressDialog dialog;
        Context context;

        AdvsList(Context context) {
                ...
                try {
                    ...
                    while (count < jsonArray.length()) {
                        ...
                        if (JO.getString("advcategory").equals("deals"))
                            advs.add(new DealsAdv(JO.getString("title"), JO.getString("bnames"), JO.getString("price"), JO.getString("discount"), JO.getString("validity"), JO.getString("shopname"),
                                    JO.getString("img1"), JO.getString("img2"), JO.getString("img3"), false));
                        else if (JO.getString("advcategory").equals("sales"))
                            advs.add(new SalesAdv(JO.getString("sname"), JO.getString("bnames"), JO.getString("discount"), JO.getString("validity"), JO.getString("shopname"),
                                    JO.getString("img1"), JO.getString("img2"), JO.getString("img3"), false));
                        else
                            advs.add(new NewAdv(JO.getString("title"), JO.getString("bnames"), JO.getString("price"), JO.getString("discount"), JO.getString("shopname"),
                                    JO.getString("img1"), JO.getString("img2"), JO.getString("img3"), false));
                        count++;
                    }
                    loadAdvs();
    ...
    public class AdvsViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int position) {
            switch (viewHolder.getItemViewType()) {
                case DEALS:
                    ...
                    configureDealsViewHolder(dealsViewHolder, position);
                    break;
                case SALES:
        ...
        void configureDealsViewHolder(final DealsViewHolder holder, int position) {
            ...
            if (dealsAdv != null) {
                ...
                if (!dealsAdv.isTimerStatus()) {
                    dealsAdv.setTimerStatus(true);
        ...
        void configureNewViewHolder(final NewViewHolder holder, int position) {
            ...
            if (newAdv != null) {
                ...
                if (newAdv.isCardStatus()) {
                    holder.imagesLayout.setVisibility(View.VISIBLE);
                    if (!newAdv.getImg1URL().equals(""))
                        Picasso.with(HomeAdvView.this).load(newAdv.getImg1URL()).into(holder.img1);
                    if (!newAdv.getImg2URL().equals(""))
                        Picasso.with(HomeAdvView.this).load(newAdv.getImg2URL()).into(holder.img2);
                    if (!newAdv.getImg3URL().equals(""))
                        Picasso.with(HomeAdvView.this).load(newAdv.getImg3URL()).into(holder.img3);
                } else
                    holder.imagesLayout.setVisibility(View.GONE);
                ...
                advCard.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (dealsAdv.isCardStatus()) {
                            ...
                            if (!dealsAdv.getImg1URL().equals(""))
                                Picasso.with(HomeAdvView.this).load(dealsAdv.getImg1URL()).into(img1);
                            if (!dealsAdv.getImg2URL().equals(""))
                                Picasso.with(HomeAdvView.this).load(dealsAdv.getImg2URL()).into(img2);
                            if (!dealsAdv.getImg3URL().equals(""))
                                Picasso.with(HomeAdvView.this).load(dealsAdv.getImg3URL()).into(img3);
                ...
                advCard.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (salesAdv.isCardStatus()) {
                            ...
                            if (!salesAdv.getImg1URL().equals(""))
                                Picasso.with(HomeAdvView.this).load(salesAdv.getImg1URL()).into(img1);
                            if (!salesAdv.getImg2URL().equals(""))
                                Picasso.with(HomeAdvView.this).load(salesAdv.getImg2URL()).into(img2);
                            if (!salesAdv.getImg3URL().equals(""))
                                Picasso.with(HomeAdvView.this).load(salesAdv.getImg3URL()).into(img3);
                            if (PREVIOUS_INDEX != -1 && PREVIOUS_INDEX != getAdapterPosition()) {
                                if (advs.get(PREVIOUS_INDEX) instanceof DealsAdv) {
                                    ((DealsAdv) advs.get(PREVIOUS_INDEX)).setCardStatus(false);
                                } else if (advs.get(PREVIOUS_INDEX) instanceof SalesAdv) {
